...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  document
    ...
    .addEventListener("input", searchOrder);
});
  ...
  realOrders.forEach((order) => {
    ...
    row.innerHTML = `
      ...
      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        <button class="text-primary hover:text-blue-700 mr-3" onclick="viewOrder(${
          order.id
        })">
          <i class="fas fa-eye"></i>
        </button>
        <button class="text-secondary hover:text-yellow-600" onclick="editOrder(${
          order.id
        })">
...
function searchOrder() {
  ...
  const filteredOrders = realOrders.filter((order) => {
    ...
    return order.id.toString().includes(query) || customerName.includes(query);
  });
...
async function updateOrderStatus() {
  ...
  try {
    console.log("Intentando actualizar orden:", orderId, "con estado:", status);
    ...
    if (!response.ok) {
      let errMessage = "Error al actualizar el pedido";

      // Solo intentar leer el body si es JSON
